...
def train(net, data_loader, loss_fn, experiment_name, valdir):
    ...
    for epoch in range(epochs):
        ...
        if current_nk > nk_best:
            torch.save(net.state_dict(), os.path.join("weights", "mobilenet_classTransfer_tripletHard_best.pth"))
            nk_best = current_nk
        torch.save(net.state_dict(), os.path.join("weights", "mobilenet_classTransfer_tripletHard_last.pth"))
...
if __name__ == '__main__':
    ...
    state_dict = torch.load(os.path.join('weights', 'mobilenet_classification_last.pth'), map_location=lambda storage, loc: storage)
